...
| `/citizenship` miniapp localized product surface | Done — standalone `CitizenshipGuide.jsx` translation system covers `en/es/pt/it/fr/ja/hi/ar/zh`; `/links`-style language menu; shared `local_npub` / `local_nsec` account creation and existing-key sign-in; durable Firestore progress/checklist state; one-question intake; edit-mode question jump menu with `Finish edits`; localized DNExpress primer, privacy accordion/modal, consulate finder panel, route results, blockers, checklist accordions, assistant drawer, report download, and official-reference accordions |
