...
export function getStoryDifficulty(cefrLevel, { includeTranslations = true } = {}) {
  ...
  const comprehensionGuidance = includeTranslations
    ? "Keep most language familiar and make new expressions understandable through context and faithful support translations."
    : "Keep most language familiar and make new expressions understandable through context.";
...
Spoken lines contain dialogue only, without a narrator, stage directions, or exercise instructions.${includeTranslations ? " Translate the meaning and tone faithfully without adding explanations." : ""}
${mode === "radio"
...
export function buildSpeakingStoryPrompt({ targetName, targetLang, difficulty, isTutorial, scenarioDirective, curriculumContext }) {
  return [
    ...
    `For each dialogue line: {"type":"sentence","character":"Sheilfer | Jiraiya | Yoruichi | Neko | Yachiru | You","gender":"male or female","tgt":"${targetName} spoken line"}`,
