...
export default function JourneyTestButton({
  npub,
  targetLang,
  lang,
  resource,
  ...
  const open = async () => {
    ...
    setFailed(false);
    try {
      const result = await unlockNextJourneyMilestoneForTesting(
        npub,
        targetLang,
      );
    ...
    <>
      <VStack align="end" spacing={1}>
        <Button
          ...
          leftIcon={<FiMic />}
          isLoading={loading}
          isDisabled={!npub || resource.loading || Boolean(opened)}
          onClick={() => {
        ...
        {failed && (
          <Text role="alert" fontSize="xs" color="var(--app-text-secondary)">
            {journeyCopy(lang, "error")}
          </Text>
        )}
      </VStack>
      {opened && (
        <JourneyRecordingModal
          key={opened.milestone}
          npub={npub}
