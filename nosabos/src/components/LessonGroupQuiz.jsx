...
export default function LessonGroupQuiz({
  ...
  function buildQuizFillPrompt() {
    ...
    if (lessonContent && Array.isArray(lessonContent)) {
      ...
      if (allWords.length > 0) {
        topicDirective = `- CONTEXT: Draw from this vocabulary pool: ${JSON.stringify([...new Set(allWords)])}. Use these words or related concepts.`;
    ...
    return [
      ...
      wantTR ? `- ${SUPPORT} translation of the full sentence.` : `- Empty translation "".`,
      "",
  ...
  useEffect(() => {
    if (npub && questionsAnswered === 0 && !qFill && !qMC && !qMA && !sTarget && !mLeft.length) {
      generateRandom();
    }
...
`.trim();
  ...
  async function checkMA() {
    ...
    const userAnswers = maLayout === "drag"
  ...
  async function checkMatch() {
    ...
    try {
      ...
      const U = userPairs
        .map(([li, ri]) => `L${li + 1} -> R${ri + 1}  (${mLeft[li]} -> ${mRight[ri] || "(none)"})`)
        .join("\n") || "(none)";
...
`.trim();
      ...
      <Modal isOpen={true} onClose={handleComplete} size="lg">
        ...
        <ModalContent bg="#1a1e2e" color="white">
          <ModalHeader textAlign="center">
            {userLanguage === "es"
              ? (passed ? "¡Examen Aprobado! 🎉" : "Examen Fallido 😔")
              : (passed ? "Quiz Passed! 🎉" : "Quiz Failed 😔")}
          </ModalHeader>
          <ModalBody>
            <VStack spacing={4}>
              <Text fontSize="2xl">
                {userLanguage === "es" ? "Puntuación" : "Score"}: {correctAnswers}/{TOTAL_QUESTIONS}
              </Text>
              <Progress
  ...
  const isLoading = loadingQFill || loadingQMC || loadingQMA || loadingQSpeak || loadingMG;
  const isGrading = loadingGFill || loadingGMC || loadingGMA || loadingMJ;

  return (
    <Box minH="100vh" bg="linear-gradient(135deg, #0f0f23 0%, #1a1e2e 50%, #16213e 100%)" py={8}>
      <VStack spacing={6} maxW="800px" mx="auto" px={4}>
        {/* Progress Header */}
        <Box w="full">
          <HStack justify="space-between" mb={2}>
            <Badge colorScheme="teal">
              {userLanguage === "es" ? "Pregunta" : "Question"} {questionsAnswered + 1}/{TOTAL_QUESTIONS}
            </Badge>
            <Badge colorScheme="purple">
              {userLanguage === "es" ? "Correctas" : "Correct"}: {correctAnswers}/{questionsAnswered}
            </Badge>
          </HStack>
          <Progress
        ...
        >
          {isLoading ? (
            <VStack spacing={4}>
              ...
              <Text color="white">{userLanguage === "es" ? "Generando pregunta..." : "Generating question..."}</Text>
            </VStack>
          ) : mode === "fill" ? (
            <VStack spacing={4} align="stretch">
              <Text fontSize="lg" color="white">{qFill}</Text>
              {hFill && <Text fontSize="sm" color="gray.400">💡 {hFill}</Text>}
              {trFill && showTranslations && <Text fontSize="sm" color="gray.500" fontStyle="italic">{trFill}</Text>}
              <Input
                value={ansFill}
                onChange={(e) => setAnsFill(e.target.value)}
                placeholder={userLanguage === "es" ? "Escribe tu respuesta..." : "Type your answer..."}
                size="lg"
                bg="whiteAlpha.100"
                color="white"
            ...
            <VStack spacing={4} align="stretch">
              <Text fontSize="lg" color="white">{qMC}</Text>
              {hMC && <Text fontSize="sm" color="gray.400">💡 {hMC}</Text>}
              {trMC && showTranslations && <Text fontSize="sm" color="gray.500" fontStyle="italic">{trMC}</Text>}
              {mcLayout === "buttons" ? (
                <RadioGroup value={pickMC} onChange={setPickMC} isDisabled={lastOk !== null}>
                  <Stack spacing={2}>
                ...
                <Text color="gray.400" fontSize="sm">(Drag variant would render here)</Text>
              )}
            </VStack>
          ) : mode === "ma" ? (
            <VStack spacing={4} align="stretch">
              <Text fontSize="lg" color="white">{qMA}</Text>
              {hMA && <Text fontSize="sm" color="gray.400">💡 {hMA}</Text>}
              {trMA && showTranslations && <Text fontSize="sm" color="gray.500" fontStyle="italic">{trMA}</Text>}
              {maLayout === "buttons" ? (
                <CheckboxGroup value={picksMA} onChange={setPicksMA} isDisabled={lastOk !== null}>
                  <Stack spacing={2}>
            ...
            <VStack spacing={4} align="stretch">
              <Text fontSize="lg" color="white">{sPrompt}</Text>
              <Text fontSize="xl" fontWeight="bold" color="cyan.300">{sStimulus}</Text>
              {sHint && <Text fontSize="sm" color="gray.400">💡 {sHint}</Text>}
              {sTranslation && showTranslations && <Text fontSize="sm" color="gray.500" fontStyle="italic">{sTranslation}</Text>}
              <Button
              ...
              >
                {isListening
                  ? (userLanguage === "es" ? "Parar Grabación" : "Stop Recording")
                  : (userLanguage === "es" ? "Empezar a Hablar" : "Start Speaking")}
              </Button>
              {transcript && (
                <Text color="white" fontSize="sm">
                  {userLanguage === "es" ? "Dijiste" : "You said"}: "{transcript}"
                </Text>
              )}
            </VStack>
          ) : mode === "match" ? (
            <VStack spacing={4} align="stretch">
              <Text fontSize="lg" color="white">{mStem || (userLanguage === "es" ? "Empareja las palabras con sus definiciones" : "Match the words with their definitions")}</Text>
              {mHint && <Text fontSize="sm" color="gray.400">💡 {mHint}</Text>}
            ...
            <Text color="white">{userLanguage === "es" ? "Tipo de pregunta desconocida" : "Unknown question type"}</Text>
          )}
          ...
          {lastOk !== null && (
            ...
            >
              {lastOk
                ? (userLanguage === "es" ? "✓ ¡Correcto!" : "✓ Correct!")
                : (userLanguage === "es" ? "✗ Incorrecto" : "✗ Incorrect")}
            </Box>
          )}
          ...
          {lastOk === null && !isLoading && (
            <Button
              ...
              isDisabled={
                ...
                (mode === "mc" && !pickMC) ||
                (mode === "ma" && (maLayout === "drag" ? !maSlots.some(s => s !== null) : picksMA.length === 0)) ||
