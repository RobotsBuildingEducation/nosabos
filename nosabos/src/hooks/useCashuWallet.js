...
export function useCashuWallet() {
  ...
  const refreshBalance = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        "[useCashuWallet] Balance refreshed:",
        totalBalance,
        "raw:",
        bal,
      );
  ...
  const initWalletService = useCallback(
    async (ndk, signer) => {
      ...
      try {
        ...
        wService.on("wallet:default", (wallet) => {
          console.log(
            "[useCashuWallet] Default wallet detected:",
            wallet?.walletId,
          );
  ...
  const deposit = useCallback(
    async (amount = 10) => {
      ...
      try {
        ...
        console.log(
          "[useCashuWallet] Invoice created:",
          pr?.substring(0, 50) + "...",
        );
        setInvoice(pr);
  ...
  const send = useCallback(
    async (recipientNpub, amount, comment = "") => {
      ...
      try {
        ...
        const proofTags = proofs.map((proof) => [
          "proof",
